...
class QFeedForward(torch.nn.Module):
    def __init__(
        ...
        input_size=1,
        lr=0.0001,
        output_size=5,   # output_size is actually the prediction horizon!!
        embedding_size=20,
        n_layers=1,
        n_steps=50,
        alpha=0.05,
        input_length=15, # input_length is the length of each input time series sample
        **kwargs
    ):
        ...
        self.LR = lr
        self.OUTPUT_SIZE = output_size  # output_size is actually the prediction horizon!!
        ...
        # self.OUTPUT_SIZE is actually the prediction horizon, 2 times self.OUTPUT_SIZE since upper and lower bound
        self.fc3 = nn.Linear(self.embedding_size,  2 * self.OUTPUT_SIZE)
    ...
    def forward(self, x):
        ...
        out = self.fc2(out)
        out = self.relu(out)
        out = self.fc3(out) # out: [batch_size, 2 * horizon]
